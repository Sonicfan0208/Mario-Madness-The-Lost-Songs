...
class MainMenuState extends MusicBeatState {
	...
	override function create() {
		...
		if (ClientPrefs.storySave[0] && ClientPrefs.storyPassv1) 
			{
				tieneflauta = true;
			}
	...
	function goToState() {
		...
		if(menuInfo[currentLevel].choices[curSelected] == 'Credits'){
				...
				case 1:
					switch (choice) {
						...
						case "Freeplay":
							new FlxTimer().start(0.4, function(tmr:FlxTimer)
							{
								FlxG.state.persistentDraw = false; openSubState(new CustomFreeplayState());
							});
					}
				case 2:
					switch (choice) {
	...
	override function closeSubState() {
		super.closeSubState();
		if (FlxG.state.subState is CustomFreeplayState || FlxG.state.subState is StoryMenuState) {
			reloadBG();
