 ...
 */
class OGbet extends FlxSpriteGroup
{
	...
	public function addText()
	{
		...
		for (character in splitWords)
		{
			...
			var isOGbet:Bool = AlphaCharacter.OGbet.indexOf(character.toLowerCase()) != -1;
			if ((isOGbet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				if (lastSprite != null)
				{
	...
	public function timerCheck(?tmr:FlxTimer = null) {
		...
		if(loopNum <= splitWords.length && splitWords[loopNum] != null) {
			...
			var isOGbet:Bool = AlphaCharacter.OGbet.indexOf(splitWords[loopNum].toLowerCase()) != -1;
...
{
	public static var OGbet:String = "abcdefghijklmnopqrstuvwxyz";
	...
	{
		super(x, y);
		var tex = Paths.getSparrowAtlas('OGbet');
		frames = tex;
